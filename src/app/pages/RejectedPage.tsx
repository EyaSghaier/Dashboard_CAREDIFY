...
import { XCircle, Mail, ArrowLeft, Sun, Moon, LogOut } from 'lucide-react';
...
const BGECGLine: React.FC<{ color: string }> = ({ color }) => {
    ...
    return (
        <div className="w-full overflow-hidden relative" style={{ height: VB_H }}>
            <style>{`@keyframes bgecg5{from{transform:translateX(0px)}to{transform:translateX(-${PERIOD}px)}}`}</style>
            <svg style={{ position: 'absolute', top: 0, left: 0, width: `${VB_W}px`, height: `${VB_H}px`, animation: `bgecg5 ${durSec}s linear infinite` }} viewBox={`0 0 ${VB_W} ${VB_H}`} preserveAspectRatio="none">
                <defs><linearGradient id="eg5" x1="0" x2="1" y1="0" y2="0"><stop offset="0%" stopColor={color} stopOpacity="0" /><stop offset="50%" stopColor={color} stopOpacity="0.18" /><stop offset="100%" stopColor={color} stopOpacity="0" /></linearGradient></defs>
                <polyline points={pts} fill="none" stroke="url(#eg5)" strokeWidth="1.6" strokeLinejoin="round" />
            </svg>
        </div>
    );
};
...
const i18n = {
    FR: {
        ...
        subtitle: 'Votre compte n\'a pas été approuvé',
        message: 'Nous sommes au regret de vous informer que votre demande d\'accès à la plateforme CAREDIFY n\'a pas été approuvée par notre équipe administrative.',
        reasons: [
            'Les informations fournies peuvent être incomplètes ou incorrectes',
            'Les documents de vérification n\'ont pas pu être validés',
            'D\'autres critères d\'admissibilité n\'ont pas été remplis',
        ],
        ...
        reapply: 'Créer un nouveau compte',
        backToLogin: 'Retour à la connexion',
        logout: 'Se déconnecter',
    },
    EN: {
        title: 'Application not approved',
        subtitle: 'Your account has not been approved',
        message: 'We regret to inform you that your application for access to the CAREDIFY platform has not been approved by our administrative team.',
        reasons: [
            'The information provided may be incomplete or incorrect',
            'Verification documents could not be validated',
            'Other eligibility criteria have not been met',
        ],
        nextSteps: 'Next steps',
        contact: 'Contact our support',
        contactDesc: 'If you believe this is an error or would like more information, contact us at:',
        reapply: 'Create a new account',
...
export const RejectedPage: React.FC = () => {
    ...
    const accent = isDark ? '#6B7280' : '#4B5563';
    ...
    const cardBorder = isDark ? 'rgba(255,255,255,0.07)' : 'rgba(107,114,128,0.14)';
    const cardShadow = isDark ? '0 32px 80px rgba(18,34,70,0.55)' : '0 32px 80px rgba(107,114,128,0.22)';
    const textColor = isDark ? '#F9FAFB' : '#0D1B3E';
    const subtitleColor = isDark ? '#9CA3AF' : '#6B7280';
    const gridColor = isDark ? 'rgba(18,34,70,0.08)' : 'rgba(107,114,128,0.055)';
    const reasonBg = isDark ? 'rgba(239,68,68,0.05)' : 'rgba(239,68,68,0.05)';
    const reasonBorder = isDark ? 'rgba(239,68,68,0.15)' : 'rgba(239,68,68,0.2)';
        ...
        <div className="min-h-screen flex flex-col relative overflow-hidden" style={{ background: pageBg }}>
            ...
            <div className="relative z-20 flex justify-between items-center px-6 pt-5">
                ...
                <div className="flex items-center gap-2">
                    <div className="flex items-center rounded-lg overflow-hidden" style={{ border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(107,114,128,0.18)'}` }}>
                    ...
                    <button onClick={toggleTheme} className="w-8 h-8 rounded-lg flex items-center justify-center"
                        style={{ background: isDark ? 'rgba(255,255,255,0.06)' : 'rgba(107,114,128,0.09)', border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(107,114,128,0.18)'}`, color: accent }}>
            ...
            <div className="relative z-10 flex flex-1 flex-col items-center justify-center px-4 py-8">
                <div className="flex flex-col items-center mb-6">
                    <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-3"
                        style={{ background: 'linear-gradient(135deg, #6B7280, #4B5563)', boxShadow: isDark ? '0 0 32px rgba(107,114,128,0.4)' : '0 0 24px rgba(107,114,128,0.3)' }}>
                ...
                <div className="w-full rounded-2xl" style={{ maxWidth: '540px', background: cardBg, border: `1px solid ${cardBorder}`, boxShadow: cardShadow }}>
                    <div className="px-8 py-8">
                        <div className="flex flex-col items-center text-center mb-6">
                            ...
                            <p className="text-sm mb-1" style={{ color: errorColor, fontWeight: 600 }}>{t.subtitle}</p>
                            ...
                            <div className="space-y-2">
                                {t.reasons.map((reason, idx) => (
                                    <div key={idx} className="flex items-start gap-3 px-4 py-3 rounded-xl" style={{ background: reasonBg, border: `1px solid ${reasonBorder}` }}>
                                        <div className="w-1.5 h-1.5 rounded-full mt-1.5 flex-shrink-0" style={{ background: errorColor }} />
                        ...
                        <div className="mb-6 p-4 rounded-xl" style={{ background: isDark ? 'rgba(255,255,255,0.03)' : 'rgba(107,114,128,0.05)', border: `1px solid ${isDark ? 'rgba(255,255,255,0.06)' : 'rgba(107,114,128,0.15)'}` }}>
                        ...
                        <div className="space-y-3">
                            <button onClick={handleLogout}
                                ...
                                style={{ background: accent, boxShadow: isDark ? '0 4px 20px rgba(107,114,128,0.4)' : '0 4px 20px rgba(107,114,128,0.35)' }}>
                                <LogOut className="w-4 h-4" /> {t.logout}
                            </button>

                            <Link to="/signup"
                                className="w-full py-3 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 transition-all hover:opacity-75"
                                style={{ background: 'transparent', border: `1px solid ${cardBorder}`, color: accent }}>
                                {t.reapply}
                            </Link>
                        </div>
